...
using MinimalChatbot.Infrastructure.Services;

namespace MinimalChatbot.Infrastructure.Middleware;
...
public class ReflectionFunctionMiddleware : IFunctionMiddleware
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (reflection.ApprovalStatus == ReflectionApprovalStatus.Rejected)
            {
                _logger.LogWarning("Reflection rejected operation {FunctionName}: {Reasoning}", functionName, reflection.Reasoning);
                context.Terminate = true;
                context.Result = $"REJECTED: {reflection.Reasoning}";
                return context.Result;
            }

            // If needs confirmation: store metadata, build ActionCard, and block
            if (reflection.ApprovalStatus == ReflectionApprovalStatus.RequiresConfirmation)
            {
                ...
                context.Result = new
                {
                    ...
                    Description = reflection.Context ?? "Operation requires confirmation",
            ...
            _logger.LogInformation("Reflection approved operation {FunctionName}: {Reasoning}", functionName, reflection.Reasoning);
            return await next().ConfigureAwait(false);
        }
        catch (Exception ex)
        {

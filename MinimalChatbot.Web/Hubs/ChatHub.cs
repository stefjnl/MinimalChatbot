...
using MinimalChatbot.Web.Services;
using System.Runtime.CompilerServices;
using System.Text;
using System.Text.Json;

namespace MinimalChatbot.Web.Hubs;
...
public class ChatHub : Hub, IDisposable
{
    ...
    public async Task StreamMessage(string message, string? conversationId)
    {
        ...
        try
        {
            ...
            var input = new ChatInput { Message = message };
            ...
            await foreach (var evt in _streamChatUseCase.ExecuteAsync(input, instructions, persistenceContext, Context.ConnectionAborted))
            {
                ...
                if (evt.EventType == ChatStreamEventType.Started)
                {
                    await Clients.Caller.SendAsync("StreamStarted", evt.ConversationId);
                }
                else if (evt.EventType == ChatStreamEventType.Message && !string.IsNullOrEmpty(evt.Content))
                {
                    fullResponse.Append(evt.Content);
                    tokenBuffer.Append(evt.Content);
                    await FlushTokenBufferAsync();
                }
                else if (evt.EventType == ChatStreamEventType.ToolCall && evt.ToolCall != null)
                {
                    await Clients.Caller.SendAsync("ToolCall", new {
                        Id = evt.ToolCall.Id,
                        ToolName = evt.ToolCall.ToolName,
                        Arguments = evt.ToolCall.Arguments,
                        Status = evt.ToolCall.Status.ToString()
                    });
                }
                else if (evt.EventType == ChatStreamEventType.Artifact && evt.Artifact != null)
                {
                    await Clients.Caller.SendAsync("Artifact", new {
                        Id = evt.Artifact.Id,
                ...
                else if (evt.EventType == ChatStreamEventType.Error && !string.IsNullOrEmpty(evt.Error))
                {
                    await Clients.Caller.SendAsync("Error", evt.Error);
                }
            }

...
public sealed class VaultPathResolver : IVaultPathResolver
{
    ...
    public async Task<string> ResolveAsync(string candidatePath, CancellationToken cancellationToken = default)
    {
        ...
        var fallback = _vaultPathNormalizer.NormalizePath(candidatePath);
        var comparisonKey = CreateMatchKey(candidatePath);
        if (string.IsNullOrEmpty(comparisonKey))
        {
            return fallback;
        }

        var fallbackKey = CreateMatchKey(fallback);
    ...
    private static string CreateMatchKey(string userInputFileName)
    {

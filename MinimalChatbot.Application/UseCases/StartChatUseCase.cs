...
public class StartChatUseCase
{
    ...
    return new Contracts.StartChatResult(conversation.Id, userMessage.Id, assistantMessage.Id, responseText, resolvedOperation, null);
    }
    ...
    private async Task<Domain.Models.FileOperation?> ResolveFileOperationAsync(Domain.Models.FileOperation? fileOperation, CancellationToken ct)
    {
        ...
        // Since FileOperation is not a record, we need to create a new instance
        return fileOperation.OperationType switch
        {
            FileOperationType.Read => FileOperation.Read(resolvedPath),
            FileOperationType.Write => FileOperation.Write(resolvedPath, fileOperation.Content ?? string.Empty),
            FileOperationType.Delete => FileOperation.Delete(resolvedPath),
            FileOperationType.Move => FileOperation.Move(fileOperation.SourcePath ?? string.Empty, resolvedPath),
            FileOperationType.ListDirectory => FileOperation.ListDirectory(resolvedPath),
            _ => fileOperation
        };
    }
    ...
    private async Task<Message> PersistUserMessageAsync(Guid conversationId, string content, CancellationToken ct)
    {
        var message = new Message
        {
            ...
            Role = MinimalChatbot.Domain.Entities.MessageRole.User,
            Content = content,
    ...
    private async Task<Message> PersistAssistantMessageAsync(Guid conversationId, string content, Domain.Models.FileOperation? fileOperation, CancellationToken ct)
    {
        var message = new Message
        {
            Id = Guid.NewGuid(),
            ConversationId = conversationId,
            Role = MinimalChatbot.Domain.Entities.MessageRole.Assistant,
            Content = content,
            Timestamp = DateTime.UtcNow,
            IsProcessing = false
        };
        ...
        if (fileOperation != null)
        {
            message.FileOperation = new FileOperationRecord
            {
                ...
                Action = fileOperation.OperationType.ToString(),

...
namespace MinimalChatbot.Tests.Infrastructure;

public class AttachmentRepositoryTests : IDisposable
{
    private readonly MinimalChatbotDbContext _dbContext;
    private readonly AttachmentRepository _repository;

    public AttachmentRepositoryTests()
    {
        var options = new DbContextOptionsBuilder<MinimalChatbotDbContext>()
            .UseInMemoryDatabase(databaseName: Guid.NewGuid().ToString())
            .Options;

        _dbContext = new MinimalChatbotDbContext(options);

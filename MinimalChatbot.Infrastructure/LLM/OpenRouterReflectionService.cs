...
public class OpenRouterReflectionService : IReflectionService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            // Validate required fields - since Reasoning is init-only, we need to create a new instance
            if (string.IsNullOrEmpty(result.Reasoning))
            {
                result = result.ApprovalStatus switch
                {
                    ReflectionApprovalStatus.Approved => ReflectionResult.Approved("Reflection completed but reason not provided", result.Context),
                    ReflectionApprovalStatus.Rejected => ReflectionResult.Rejected("Reflection completed but reason not provided", result.Context),
                    ReflectionApprovalStatus.RequiresConfirmation => ReflectionResult.RequiresConfirmation("Reflection completed but reason not provided", result.Context),
                    _ => ReflectionResult.Approved("Reflection completed but reason not provided", result.Context)
                };
            }

            _logger.LogInformation(
                "Reflection completed for {ToolName}: Status={Status}, Reasoning={Reasoning}",
                toolName,
    ...
    private static ReflectionResult CreateApprovedResult(string reasoning)
    {
        return ReflectionResult.Approved(reasoning, "Operation approved due to reflection service limitation");
    }
